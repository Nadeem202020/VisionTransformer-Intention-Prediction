...
def transform_points(points: np.ndarray, transform_matrix: np.ndarray) -> np.ndarray:
    ...
    homogeneous_points = np.hstack((points[:, :3], np.ones((points.shape[0], 1)))) # Ensure only 3D points
    transformed_points = (transform_matrix @ homogeneous_points.T).T
    return transformed_points[:, :3]

# --- BEV Generation Utilities ---
def get_ego_centric_transform_matrix(ego_translation_xy: np.ndarray, ego_yaw: float) -> np.ndarray:
    ...
    cos_yaw, sin_yaw = np.cos(-ego_yaw), np.sin(-ego_yaw) # Rotate by -yaw
    rotation_matrix = np.array([[cos_yaw, -sin_yaw],
                                [sin_yaw,  cos_yaw]])
    translation_vector = -rotation_matrix @ ego_translation_xy # Translate by negative rotated offset
    
    T = np.eye(3) # Homogeneous transformation matrix
...
def world_to_bev_pixel(points_world_xy: np.ndarray, ego_tf_matrix: np.ndarray) -> np.ndarray:
    ...
    points_ego_homo = (ego_tf_matrix @ points_world_homo.T).T # Transform to ego frame
    points_ego_xy = points_ego_homo[:, :2]

    # Convert ego coordinates to pixel coordinates
    # Ego X (forward) -> BEV Y-axis (image rows, typically downwards)
    # Ego Y (left)    -> BEV X-axis (image columns, typically rightwards)
    pixel_x = BEV_PIXEL_OFFSET_X + points_ego_xy[:, 1] / VOXEL_SIZE_M
    pixel_y = BEV_PIXEL_OFFSET_Y - points_ego_xy[:, 0] / VOXEL_SIZE_M # Minus because pixel Y increases downwards
...
def create_intentnet_lidar_bev(points_list: list[np.ndarray | None],
    ...
    for sweep_idx in range(num_loaded_sweeps):
        ...
        # Filter points outside the BEV grid and Z-range
        valid_mask = (
        ...
        # Calculate height slice index for each point
        z_slice_idx = np.floor((z_ego_filtered - Z_MIN) / (Z_MAX - Z_MIN) * LIDAR_HEIGHT_CHANNELS).astype(int)
        z_slice_idx = np.clip(z_slice_idx, 0, LIDAR_HEIGHT_CHANNELS - 1) # Ensure within bounds

        channel_offset = sweep_idx * LIDAR_HEIGHT_CHANNELS
        for h_idx in range(LIDAR_HEIGHT_CHANNELS):
            # Points belonging to the current height slice
            h_mask = (z_slice_idx == h_idx)
            if np.any(h_mask):
                # Efficiently update BEV with max intensity using np.maximum.at (handles duplicates)
                np.maximum.at(
...
def rasterize_map_ego_centric(map_json_path: str, current_ego_pose: pd.Series) -> np.ndarray:
    ...
    try:
        ego_yaw = R.from_quat(ego_q).as_euler('xyz')[2] # Z-axis rotation (yaw)
    except ValueError:
    ...
    def to_bev_pixel_local(points_world_xyz_list: list[dict]) -> np.ndarray:
        ...
        # Filter points outside the final BEV grid dimensions
        valid_bev_mask = (
    ...
    bev_map_uint8 = np.zeros(bev_map_shape, dtype=np.uint8) # Use uint8 for OpenCV drawing functions

    # Rasterize Lane Segments
    for _, lane in lane_segments.items():
        ...
        # Drivable Area & Special Lanes (Intersection, Bus)
        if len(left_px) > 1 and len(right_px) > 1:
            # Ensure polygon has at least 3 points for cv2.fillPoly
            polygon_points = np.vstack([left_px, np.flipud(right_px)])
            if polygon_points.shape[0] >= 3:
                cv2.fillPoly(bev_map_uint8[0], [polygon_points.reshape(-1, 1, 2)], color=1) # Channel 0: Drivable
                if lane.get("is_intersection", False):
                    cv2.fillPoly(bev_map_uint8[4], [polygon_points.reshape(-1, 1, 2)], color=1) # Channel 4: Intersection
                if lane.get("lane_type") == "BUS":
                    cv2.fillPoly(bev_map_uint8[5], [polygon_points.reshape(-1, 1, 2)], color=1) # Channel 5: Bus Lane
    ...
    # Rasterize Pedestrian Crossings
    for _, cw in crosswalks.items():
        poly_world = cw.get('polygon', [])
        if poly_world:
            pts_px = to_bev_pixel_local(poly_world)
            if len(pts_px) >= 3:
                cv2.fillPoly(bev_map_uint8[3], [pts_px.reshape(-1, 1, 2)], color=1) # Channel 3: Pedestrian Crossing

    return bev_map_uint8.astype(np.float32) # Convert to float for model input

# --- Ground Truth Preparation ---
def prepare_gt_for_frame(current_ts_ns: int, gt_df_with_intent: pd.DataFrame,
                         static_map) -> dict[str, torch.Tensor]: # static_map type can be av2.map.map_api.ArgoverseStaticMap or None
    """
    ...
    frame_gt = gt_df_with_intent[
        ...
        (gt_df_with_intent['heuristic_intent'] != -1) # Use pre-calculated labels
    ]

    if 'heuristic_intent' not in frame_gt.columns:
        # This is a critical error if pre-computation was expected.
    ...
    for _, box_row in frame_gt.iterrows():
        try:
            ...
            w, l = abs(box_row['width_m']), abs(box_row['length_m']) # Ensure positive dimensions
            quat = box_row[['qx', 'qy', 'qz', 'qw']].values
            heading_rad = R.from_quat(quat).as_euler('xyz', degrees=False)[2] # Yaw angle

            gt_boxes_xywha_list.append([cx, cy, w, l, heading_rad])
            gt_intentions_list.append(int(box_row['heuristic_intent']))
        except (ValueError, KeyError) as e:
            # print(f"Warning (GT Prep): Skipping box due to error: {e} for track {box_row.get('track_uuid', 'N/A')}")
            # Ensure lists remain synchronized if an error occurs mid-append.
            if len(gt_boxes_xywha_list) > len(gt_intentions_list):
                gt_boxes_xywha_list.pop()
            continue # Skip this box

    if not gt_boxes_xywha_list: # No valid GT boxes found for this frame
        return {'boxes_xywha': torch.empty((0, 5), dtype=torch.float32),
                'intentions': torch.empty((0,), dtype=torch.long)}
...
def decode_box_predictions(box_preds_rel: torch.Tensor, anchors_xywha: torch.Tensor) -> torch.Tensor:
    ...
    pred_cy = dy * anchor_l + anchor_cy # Assuming dy is relative to anchor length
...
def apply_nms(boxes_xywha: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.2) -> torch.Tensor:
    ...
    x1 = cx - w / 2
    y1 = cy - l / 2 # Using length for BEV "height"
    x2 = cx + w / 2
    y2 = cy + l / 2
...
def random_flip_bev(lidar_bev: np.ndarray, map_bev: np.ndarray,
    ...
    if random.random() < 0.5: # 50% chance
        lidar_bev = np.ascontiguousarray(np.flip(lidar_bev, axis=2)) # Flip width axis
        map_bev = np.ascontiguousarray(np.flip(map_bev, axis=2))
        if gt_boxes_xywha.shape[0] > 0:
            gt_boxes_xywha[:, 1] *= -1  # Flip Ego Y coordinate
            gt_boxes_xywha[:, 4] *= -1  # Flip heading angle
            # Normalize angle to [-pi, pi]
            gt_boxes_xywha[:, 4] = np.arctan2(np.sin(gt_boxes_xywha[:, 4]), np.cos(gt_boxes_xywha[:, 4]))
        if gt_intentions.shape[0] > 0:
            # Flip intentions (Left <-> Right turns and lane changes)
            # Assumes INTENTIONS_MAP is available from constants.py
            mapping = {
            ...
            original_intentions = gt_intentions.copy() # Avoid modifying in place during iteration
...
def random_rotate_bev(lidar_bev: np.ndarray, map_bev: np.ndarray, gt_boxes_xywha: np.ndarray,
    ...
    if random.random() < 0.5: # 50% chance
        ...
        # BEV image center for rotation (pixel coordinates)
        center_px_x = GRID_WIDTH_PX / 2.0 # Note: OpenCV uses (cols, rows) for center
        center_px_y = GRID_HEIGHT_PX / 2.0 # Using image center, not BEV_PIXEL_OFFSET_X/Y for image rotation
                                           # BEV_PIXEL_OFFSET_X/Y is for world-to-pixel, this is image-to-image
        ...
        if gt_boxes_xywha.shape[0] > 0:
            # Rotate GT box centers (cx, cy) in ego-centric metric space
            cx, cy = gt_boxes_xywha[:, 0].copy(), gt_boxes_xywha[:, 1].copy()
            cos_a, sin_a = np.cos(angle_rad), np.sin(angle_rad) # Rotation matrix for points
    ...
    if random.random() < 0.5: # 50% chance
        ...
        def scale_all_channels(bev_tensor: np.ndarray) -> np.ndarray:
            ...
            for i in range(bev_tensor.shape[0]):
                ...
                # Pad or crop to original dimensions, centered
        ...
        if gt_boxes_xywha.shape[0] > 0:
            # Scale cx, cy, w, l. Heading remains unchanged by scaling.
            gt_boxes_xywha[:, :4] *= scale_factor
    return lidar_bev, map_bev, gt_boxes_xywha

def random_bev_dropout(lidar_bev: np.ndarray, map_bev: np.ndarray,
                       dropout_prob: float = 0.1,
                       patch_size_range: tuple[int, int] = (20, 50), # Increased patch size for BEV
    ...
    if random.random() < dropout_prob:
        ...
        for _ in range(num_patches):
            ...
            # Ensure patch start points allow patch to be within image boundaries
            ...
            map_bev[:, start_y:start_y+patch_h, start_x:start_x+patch_w] = 0.0 # Assumes map is float; if uint8, use 0
...
def augment_bev(lidar_bev: np.ndarray, map_bev: np.ndarray, gt_dict: dict
    ...
    # Ensure GTs are NumPy for augmentation functions
...
def generate_anchors(bev_height: int = GRID_HEIGHT_PX, bev_width: int = GRID_WIDTH_PX,
                     ...
                     offset_x_px: float = BEV_PIXEL_OFFSET_X, # Renamed for clarity
                     offset_y_px: float = BEV_PIXEL_OFFSET_Y  # Renamed for clarity
                     ) -> torch.Tensor:
    """
    ...
    # Create grid centers in pixel coordinates for the feature map
    grid_y_fm, grid_x_fm = torch.meshgrid(torch.arange(feature_map_h), torch.arange(feature_map_w), indexing='ij')
    
    # Center of each feature map cell in original BEV pixel coordinates
    ...
    # Convert BEV pixel centers to ego-centric metric coordinates
    # Recall: pixel_x = offset_x_px + ego_y / voxel_size  => ego_y = (pixel_x - offset_x_px) * voxel_size
    #         pixel_y = offset_y_px - ego_x / voxel_size  => ego_x = (offset_y_px - pixel_y) * voxel_size
    center_ego_y_m = (center_pixel_x - offset_x_px) * voxel_size
    center_ego_x_m = (offset_y_px - center_pixel_y) * voxel_size
    
    # Stack to get [Hf, Wf, 2] tensor of (ego_x, ego_y) centers
    centers_m_ego = torch.stack([center_ego_x_m, center_ego_y_m], dim=-1)

    all_anchors_list = []
    for (w_anchor_m, l_anchor_m, yaw_anchor_rad) in anchor_configs:
        # Anchor dimensions: [Hf, Wf, 3] -> (w, l, yaw)
        ...
        # Concatenate centers and dims: [Hf, Wf, 5]
        anchors_at_config = torch.cat([centers_m_ego, anchor_dims_expanded], dim=-1)
        all_anchors_list.append(anchors_at_config.reshape(-1, 5)) # Flatten to [Hf*Wf, 5]

    # Concatenate all anchor types: [NumAnchorTypes * Hf*Wf, 5]
    # Then reshape and permute to group by location: [Hf*Wf, NumAnchorTypes, 5] -> flatten
    final_anchors = torch.cat(all_anchors_list, dim=0) # This stacks them, need to interleave
    
    # To interleave by location:
    # Create a list of [Hf*Wf, 5] tensors, one for each anchor type
    # Then stack them along a new dimension and reshape correctly.
    anchors_per_location = []
    for (w_m, l_m, r_rad) in anchor_configs:
        dims_tensor = torch.tensor([w_m, l_m, r_rad], dtype=torch.float32).unsqueeze(0).repeat(centers_m_ego.shape[0]*centers_m_ego.shape[1], 1) # [Hf*Wf, 3]
        current_anchors = torch.cat([centers_m_ego.reshape(-1,2), dims_tensor], dim=1) # [Hf*Wf, 5]
        anchors_per_location.append(current_anchors)
    
    # Stack and interleave:
    # This results in [NumAnchorTypes, Hf*Wf, 5]
    stacked_anchors = torch.stack(anchors_per_location, dim=0)
    # Transpose and reshape to [Hf*Wf*NumAnchorTypes, 5] where anchors for same location are grouped
    final_anchors_interleaved = stacked_anchors.transpose(0,1).reshape(-1,5)

    # print(f"Generated {final_anchors_interleaved.shape[0]} anchors ({len(anchor_configs)} types per location).")
...
def calculate_ap(recall: np.ndarray, precision: np.ndarray) -> float:
    ...
    # Append sentinel values for full recall range
